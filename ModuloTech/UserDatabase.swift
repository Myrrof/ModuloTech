...
//  ModuloTech
//
//  Created by Laurent Cheng on 28/07/2021.
//
...
extension UserDatabase {
    ...
    func deleteLightById(id: Int, completion: () -> ()) {
        guard let db = db else { return }
        do {
            let filter = table.light.filter(Expression<Int>("id") == id)
            let delete = try db.run(filter.delete())
            if delete == 1 {
                completion()
            }
        } catch { }
...
extension UserDatabase {
    ...
    func deleteRollerShutterById(id: Int, completion: () -> ()) {
        guard let db = db else { return }
        do {
...
extension UserDatabase {
    ...
    func deleteHeaterById(id: Int, completion: () -> ()) {
        guard let db = db else { return }
        do {
